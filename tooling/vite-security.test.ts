...
import { isLoopbackRemoteAddress, isTrustedBoardProxyRequest, prepareBoardProxyRequest } from "./vite-security";
...
describe("development board proxy security", () => {
  ...
  it("adds the configured human token only to a trusted request", () => {
    ...
    expect(trustedProxyRequest.setHeader).toHaveBeenCalledWith("authorization", "Bearer private-human-token");
