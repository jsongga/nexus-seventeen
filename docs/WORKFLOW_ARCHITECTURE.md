...
- **One level** — validation rejects a child plan that declares children (`validateWorkflowPlanChildren` in [`validate/plans.ts`](../src/shared/task-board-contract/validate/plans.ts)).
- **Repositories belong to a Project** — a Project owns one or more `repositories` rows, a work item names one, and a declared child may target a different one from its siblings. An agent serves exactly one (`agents.repository_id`; null means the Project's primary), and stage dispatch matches role and repository together, so a repository with no agent for a stage's role blocks rather than running in the wrong tree. Add repositories with `POST /v1/projects/{projectId}/repositories`.
...
| `projects`               | Adds `repo_path`. `repoPath` is optional at project creation; as a compatibility fallback, omission copies `description` into it (`parseBoardCreateProject` in [`validate/requests.ts`](../src/shared/task-board-contract/validate/requests.ts) and `ProjectsCollaborator.createProject` in [`projects.ts`](../src/server/task-board/collaborators/projects.ts)). |
...
| Concern                                         | Source                                                                                                                                                                                                    |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Declaration and split validation                | [`validate/plans.ts`](../src/shared/task-board-contract/validate/plans.ts)                                                                                                                                |
