...
describe('groupWorkItems', () => {
  it('places ordinal-sorted children directly after their parent with progress and phase hints', () => {
    ...
    const migrate = workItem('migrate', { parentWorkItemId: parent.id, phase: 'migrate', childOrdinal: 1, state: 'merged' });
    const contract = workItem('contract', { parentWorkItemId: parent.id, phase: 'contract', childOrdinal: 2, state: 'queued' });
    const abandoned = workItem('abandoned', { parentWorkItemId: parent.id, childOrdinal: 3, state: 'abandoned' });
    const deadLettered = workItem('dead-lettered', { parentWorkItemId: parent.id, childOrdinal: 4, state: 'dead_letter' });
  ...
  it('keeps an orphan child visible as a root with its own dependency hint', () => {
    ...
    expect(groupWorkItems([orphan])).toEqual([expect.objectContaining({
      workItem: orphan,
      depth: 0,
