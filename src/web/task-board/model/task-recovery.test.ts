        ...
        for (const hasEligibleReplacement of [true, false]) {
          it(`derives ${status}, ${assigned ? "assigned" : "unassigned"}, ${workflowBound ? "workflow-bound" : "standalone"}, ${hasEligibleReplacement ? "with" : "without"} an eligible replacement`, () => {
            ...
            const eligibleAgentIds = assigned
              ? hasEligibleReplacement
                ? ["current-agent", "replacement-agent"]
                : ["current-agent"]
              : hasEligibleReplacement
                ? ["replacement-agent"]
                : [];
...
describe("task agent picker selection", () => {
  ...
  it("preserves an explicit selection across polls and clears it for another task", () => {
    const selected = explicitAgentPickerSelection(initialAgentPickerSelection("task-one", "agent-one"), "agent-three");
...
it("drops an explicit selection when the agent leaves the eligible set", () => {
  const selected = explicitAgentPickerSelection(initialAgentPickerSelection("task-one", "agent-one"), "agent-two");
  expect(syncAgentPickerSelection(selected, "task-one", "agent-one", ["agent-one", "agent-three"])).toEqual({
    taskId: "task-one",
    agentId: "agent-one",
    explicit: false,
  });
  expect(syncAgentPickerSelection(selected, "task-one", "agent-one", ["agent-one", "agent-two"])).toBe(selected);
});
