...
describe("action error lifecycle", () => {
  ...
  it("sets, dismisses, and replaces an error only when the same action retries", () => {
    ...
    expect(assigningAgain).toEqual([{ context: "question:two:answer", error: "The answer was rejected." }]);
    ...
    expect(
      actionErrorReducer(replacedAssignmentError, {
        type: "dismissed",
        context: "question:two:answer",
      })
    ).toEqual([{ context: "task:one:assign", error: "Choose another agent." }]);
  });
  ...
  it("keeps action failures unchanged across failed and successful poll transitions", () => {
    ...
    expect(pollRecovered.actionErrors).toEqual([{ context: "task:one:assign", error: "The task changed." }]);
  });

  it("retains many independent errors, presents the newest first, and dismisses by key", () => {
    const contexts = Array.from({ length: 16 }, (_, index) => actionErrorContexts.agentSend(`agent-${index}`));
    const accumulated = contexts.reduce(
      (state, context, index) =>
        actionErrorReducer(state, {
          type: "failed",
          context,
          error: `Failure ${index}`,
        }),
...
describe("action error contexts", () => {
  it("gives every action an entity-scoped operation key", () => {
    ...
    expect(actionErrorContexts.taskRecoveryReassign("task:one")).not.toBe(actionErrorContexts.taskAssign("task:one"));
    expect(actionErrorContexts.workItemRejectPlan("work-item:one")).not.toBe(
      actionErrorContexts.workItemCancel("work-item:one")
    );
    expect(actionErrorContexts.workItemAnswer("work-item:one", "question:one")).not.toBe(
      actionErrorContexts.workItemAnswer("work-item:one", "question:two")
    );
  });
...
describe("action error taxonomy", () => {
  it.each([
    [
      "TASK_TERMINAL",
      "This task is already completed or cancelled and cannot be recovered. Refresh to see its current state.",
    ],
  ...
  it("distinguishes authentication, authorization, validation, generic conflict, and server failures", () => {
    expect(actionErrorMessage(new BoardApiError("expired", 401, "UNAUTHENTICATED"))).toBe(
      "Your sign-in has expired. Sign in again, then retry this change."
    );
    expect(actionErrorMessage(new BoardApiError("wrong group", 403, "FORBIDDEN"))).toBe(
      "Your account does not have permission to make this change."
    );
    expect(actionErrorMessage(new BoardApiError("Choose a project.", 400, "PROJECT_REQUIRED"))).toBe(
      "Check this change: Choose a project."
    );
    expect(actionErrorMessage(new BoardApiError("stale", 409, "AGENT_VERSION_CONFLICT"))).toBe(
      "The board changed in another session. Refresh before trying again."
    );
    expect(actionErrorMessage(new BoardApiError("database unavailable", 503, "UNAVAILABLE"))).toBe(
      "The board could not save this change. database unavailable"
    );
  });
