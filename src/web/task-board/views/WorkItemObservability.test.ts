...
describe('default observability view', () => {
  it('renders unread notifications and dispatches mark-read before refreshing', async () => {
    const markup = renderToStaticMarkup(createElement(NotificationsBlock, {
      notifications,
      loading: false,
      error: null,
      markingId: null,
    ...
    expect(markNotificationRead.mock.invocationCallOrder[0]).toBeLessThan(getNotifications.mock.invocationCallOrder[0]!);
  });
  ...
  it('renders work-item stage, state age, round, and heartbeat freshness', () => {
    const currentMarkup = renderToStaticMarkup(createElement(WorkItemRow, {
      workItem: workItem({
    ...
    const staleMarkup = renderToStaticMarkup(createElement(WorkItemRow, {
...
describe('work-item timeline and audit', () => {
  ...
  it('renders gate actions with abbreviated monospace shas and an explicit empty state', () => {
    ...
    const emptyMarkup = renderToStaticMarkup(createElement(AuditSection, {
