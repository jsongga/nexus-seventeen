...
describe("ledger background refresh", () => {
  ...
  it("reloads both ledgers on revision change without blanking rendered data", async () => {
    ...
    const pendingFindings = new Promise<RawFindingsLedger>((resolve) => {
      resolveFindings = resolve;
    });
    const pendingParks = new Promise<RawParksLedger>((resolve) => {
      resolveParks = resolve;
    });
    const getFindingsLedger = vi
      .fn()
      .mockResolvedValueOnce(initialFindings)
      .mockImplementationOnce(() => pendingFindings);
    const getParksLedger = vi
      .fn()
