...
describe('ledgers page panels', () => {
  ...
  it('renders age-sorted open parks, resolution history, links, and the exact history notice', () => {
    const markup = renderToStaticMarkup(createElement(ParksLedgerSection, {
      ledger: parks,
  ...
  it('renders explicit empty states for both datasets', () => {
    const findingsMarkup = renderToStaticMarkup(createElement(FindingsLedgerSection, {
      ledger: { categories: [], perProject: [], recent: [] },
    }));
    const parksMarkup = renderToStaticMarkup(createElement(ParksLedgerSection, {
