...
describe('work-item confirmation surfaces', () => {
  ...
  it('pins the parent approval dialog and action names', () => {
    ...
    const actions = renderToStaticMarkup(createElement(FinalApprovalActions, {
      busy: false,
    ...
    const confirmation = renderToStaticMarkup(createElement(FinalRejectionForm, {
  ...
  it('pins that a child gets no parent affordances', () => {
    ...
    const markup = renderToStaticMarkup(createElement(WorkItemDetail, {
      ...
      initialFamily: {
        state: 'ready',
        children: [{ ...expandChild, phase: null, state: 'final_approval' }, {
          ...expandChild,
          id: 'sibling-child',
          phase: null,
...
describe('decomposition detail sections', () => {
  ...
  it('renders the ordered children table with phase, project, state, attestation, and links', () => {
    ...
    for (const text of ['Children', 'Ordinal', 'Phase', 'Project', 'State', 'Attestation', 'Expand', 'Contract', 'Provider project', 'Attested', 'Not required', 'Open child']) {
  ...
  it('keeps loading and empty candidate sections invisible', () => {
    const loading = renderToStaticMarkup(createElement(ChildrenSection, {
      children: [],
      projects: [],
      state: 'loading',
      error: null,
    ...
    const empty = renderToStaticMarkup(createElement(ChildrenSection, {
      children: [],
      projects: [],
      state: 'ready',
      error: null,
  ...
  it('pins the attestation dialog form names and Contract dependency statuses', () => {
    const form = renderToStaticMarkup(createElement(AttestDeploymentForm, {
    ...
    const gate = renderToStaticMarkup(createElement(ContractAttestationGate, {
    ...
    const unknown = renderToStaticMarkup(createElement(ContractAttestationGate, {
      statuses: [{ child: expandChild, direct: true, ready: true }],
      phase: 'unrecognized',
      state: 'ready',
      error: null,
    ...
    const failed = renderToStaticMarkup(createElement(ContractAttestationGate, {
      statuses: [],
      phase: 'contract',
      state: 'error',
  ...
  it('pins resume and blocks archiving a merged unattested phased child', () => {
    const markup = renderToStaticMarkup(createElement(WorkItemFooterActions, {
      busy: false,
  ...
  it('keeps candidate families silent until children are known and makes a known-parent 500 recoverable', () => {
    const ok = async () => ({ ok: true as const });
    const renderCandidate = (initialFamily: {
      state: 'ready' | 'error';
      children: BoardChildWorkItem[];
      dependencies: [];
      error: string | null;
      status?: number;
    }, knownParent = false) => renderToStaticMarkup(createElement(WorkItemDetail, {
    ...
    const notFound = renderCandidate({ state: 'error', children: [], dependencies: [], error: 'Not found.', status: 404 });
    const failed = renderCandidate({ state: 'error', children: [], dependencies: [], error: 'Server failed.', status: 500 }, true);
  ...
  it('pins phased child-failure cancellation and unphased resume', () => {
    const ok = async () => ({ ok: true as const });
    const renderFamily = (phase: BoardChildWorkItem['phase']) => renderToStaticMarkup(createElement(WorkItemDetail, {
      workItem: parkedWorkItem,
      snapshotRevision: 1,
      projectName: 'Project one',
      projects: [],
  ...
  it('renders Retry in a child deployment-attestation failure section', () => {
    ...
    const markup = renderToStaticMarkup(createElement(WorkItemDetail, {
      ...
      openQuestion: null,
      client: {} as TaskBoardClient,
      initialFamily: { state: 'error', children: [], dependencies: [], error: 'Attestation lookup failed.', status: 500 },
      busy: false,
      onClose: vi.fn(),
      onAnswer: ok,
      onConfirm: ok,
