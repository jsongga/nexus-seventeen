...
describe("decomposition client", () => {
  it("parses children and dependencies and sends attestation and resume actions", async () => {
    ...
    await expect(client.getWorkItemChildren("parent-one")).resolves.toEqual([
    ...
    await expect(
      client.attestDeployment(child.workItemId, { note: "  Production rollout complete.  " })
    ).resolves.toEqual({
    ...
    await expect(client.resumeWorkItem("parent-one")).resolves.toMatchObject({
      id: "parent-one",
      state: "coordinating",
    });
    ...
    expect(calls).toEqual([
      ...
      [
        `https://board.example.test/v1/work-items/${child.workItemId}/attest-deploy`,
        "POST",
        { note: "Production rollout complete." },
      ],
