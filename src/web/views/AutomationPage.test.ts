...
describe("automation executor display", () => {
  ...
  it.each([{ kind: "disabled" } as const, { kind: "agent_type", agentTypeId: "testing-verifier" } as const])(
    "offers machine verification for testing when the saved executor is $kind",
    (executor) => {
      const configuration: AutomationConfiguration = {
        id: "company-default",
        agentTypes:
