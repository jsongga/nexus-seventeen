...
import type { TaskDetailDraftAction, TaskDetailDraftState } from "../model/task-detail-drafts";
import type { BoardAgent, BoardQuestion, BoardRun, BoardTask } from "../types";
...
function TaskPhases({ task }: { task: BoardTask }) {
  ...
  for (const phase of phases) {
    if (phase.parallelGroup)
      parallelCounts.set(phase.parallelGroup, (parallelCounts.get(phase.parallelGroup) ?? 0) + 1);
  }

  return (
    <section className="border-b border-line px-4 py-4 sm:px-5" aria-labelledby="task-phases-heading">
      <h3 id="task-phases-heading" className="text-xs font-semibold text-ink">
        Phases
      </h3>
      {phases.length === 0 ? (
        <p className="mt-3 text-sm leading-6 text-muted">
          The agent will add phases and an estimate after reviewing the task.
        </p>
      ) : (
...
export function TaskDetail({
  ...
  const recoveryDescribedBy =
    [busy || recovery?.reassign.disabledReason ? recoveryHelpId : null, selectedWorkerOffline ? offlineNoticeId : null]
  ...
  function selectAgent(nextAgentId: string): void {
    setAgentSelection((current) =>
      explicitAgentPickerSelection(syncAgentPickerSelection(current, task.id, defaultAgentId), nextAgentId)
    );
  }
  ...
  return (
    <section aria-label={`Task details: ${task.title}`}>
      <Card className="overflow-hidden">
        <header className="border-b border-line px-4 py-5 sm:px-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {task.expectedAgentMinutes !== null ? (
              <span className="text-[11px] text-muted">About {task.expectedAgentMinutes} agent min</span>
            ) : null}
          </div>
          <h2
        ...
        {mutationAffordances.answerQuestion && openQuestion ? (
          <section className="border-b border-caution-fill/30 bg-caution-soft/55 px-4 py-4 sm:px-5">
            ...
            <Button
              ...
              onClick={() =>
                void runAction(actionErrorContexts.taskAnswer(task.id, openQuestion.id), () =>
                  onAnswer(openQuestion.id, drafts.answer.trim())
                ).then((saved) => {
                  if (saved) dispatchDraft({ type: "answer-changed", value: "" });
                })
              }
            >
        ...
        {mutationAffordances.decideHumanCheck ? (
          <section className="border-b border-caution-fill/30 bg-caution-soft/45 px-4 py-4 sm:px-5">
            <div className="flex items-center gap-2 text-caution">
              ...
              <h3 className="text-xs font-semibold">Human release decision</h3>
            </div>
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              <Button
                ...
                onClick={() =>
                  void runAction(actionErrorContexts.taskHumanCheckApprove(task.id), () =>
                    onDecideHumanCheck("completed", drafts.decisionRationale.trim())
                  ).then((saved) => {
                    if (saved) dispatchDraft({ type: "rationale-changed", value: "" });
                  })
                }
              >
                Approve
              ...
              <Button
                ...
                disabled={busy || drafts.decisionRationale.trim().length === 0}
                onClick={() =>
                  void runAction(actionErrorContexts.taskHumanCheckRequestChanges(task.id), () =>
                    onDecideHumanCheck("failed", drafts.decisionRationale.trim())
                  ).then((saved) => {
                    if (saved) dispatchDraft({ type: "rationale-changed", value: "" });
                  })
                }
          ...
          <section className="space-y-3 px-4 py-4 sm:px-5" aria-label="Task recovery actions">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-muted">
                Retry the same assignment, choose another eligible agent, or return standalone work to the backlog.
              </p>
            </div>
            {recovery.retry ? (
              <Button
                ...
                disabled={busy}
                onClick={() => void runAction(actionErrorContexts.taskRetry(task.id), onRetry)}
              >
                Retry
              </Button>
            ) : null}
              ...
              >
                ...
                {recoveryAgents.map((agent) => (
                  <option key={agent.id} value={agent.id}>
                    {assignmentAgentOptionLabel(agent)}
                  </option>
                ))}
              </select>
            ...
            {recovery.backlog ? (
              <Button
                ...
                disabled={busy}
                onClick={() => void runAction(actionErrorContexts.taskRecoveryBacklog(task.id), onRecoveryBacklog)}
              >
                Return to backlog
              </Button>
            ) : null}
            {busy || recovery.reassign.disabledReason ? (
              <p id={recoveryHelpId} className="text-xs leading-5 text-muted">
                {busy
                  ? "Recovery actions are temporarily unavailable while a board change is in progress."
                  : recovery.reassign.disabledReason}
              </p>
            ) : null}
          </section>
            ...
            <div className="space-y-3">
              {eligibleAgents.length > 0 ? (
                <div>
                  <select
                    className={inputClass}
                    aria-label={task.kind === "manager_review" ? "Assign manager" : "Assign agent"}
                    aria-describedby={selectedWorkerOffline ? offlineNoticeId : undefined}
                    value={agentId}
                    onChange={(event) => selectAgent(event.target.value)}
                  >
              ...
              <Button
                ...
                icon={<UserRoundCheck size={16} />}
                aria-describedby={selectedWorkerOffline ? offlineNoticeId : undefined}
                disabled={busy || agentId.length === 0 || (queuedUnclaimed && !assigneeChanged)}
                onClick={() => void runAction(actionErrorContexts.taskAssign(task.id), () => onAssign(agentId))}
              >
                {queuedUnclaimed
                  ? task.kind === "manager_review"
                    ? "Reassign manager and wake"
                    : "Reassign and wake agent"
                  : task.kind === "manager_review"
                    ? "Assign manager and wake"
              ...
              {queuedUnclaimed ? (
                <Button
                  ...
                  icon={<ArrowLeft size={16} />}
                  disabled={busy}
                  onClick={() => void runAction(actionErrorContexts.taskReturnToBacklog(task.id), onReturnToBacklog)}
                >
                  Return to backlog
                </Button>
              ) : null}
        ...
        {actionErrors.errors.length > 0 ? (
          <div className="border-t border-line px-4 py-4 sm:px-5">
            <InlineActionErrors errors={actionErrors.errors} onDismiss={actionErrors.dismiss} />
          </div>
        ) : null}
        ...
        {mutationAffordances.interrupt && activeRun ? (
          <section className="flex items-center justify-between gap-3 px-4 py-4 sm:px-5">
            ...
            <Button
              ...
              disabled={busy}
              onClick={() =>
                void runAction(actionErrorContexts.taskInterrupt(task.id, activeRun.id), () =>
                  onInterrupt(activeRun.id)
                )
              }
            >
              Interrupt
