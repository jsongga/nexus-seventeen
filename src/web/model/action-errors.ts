...
function actionContext(entityKind: string, entityId: string, operation: string, ...relatedEntities: string[]): string {
  return [entityKind, entityId, ...relatedEntities, operation].map((segment) => encodeURIComponent(segment)).join(":");
}
...
export const actionErrorContexts = {
  ...
  workItemAnswer: (workItemId: string, questionId: string) =>
    actionContext("work-item", workItemId, "answer", "question", questionId),
  workItemConfirmPlan: (workItemId: string, planRevisionId: string) =>
    actionContext("work-item", workItemId, "confirm-plan", "plan", planRevisionId),
...
export function isDialogAnchoredActionContext(context: string): boolean {
  return (
    context === actionErrorContexts.projectCreate ||
...
export function actionErrorReducer(state: ActionErrorState, event: ActionErrorEvent): ActionErrorState {
  if (event.type === "started" || event.type === "dismissed") {
    return withoutContext(state, event.context);
  }
  return [...withoutContext(state, event.context), { context: event.context, error: event.error }];
}
...
export function errorPipelineReducer(state: ErrorPipelineState, event: ErrorPipelineEvent): ErrorPipelineState {
  ...
  const actionEvent: ActionErrorEvent =
...
export const mutationNetworkError =
  "Couldn't reach the board — your change was not saved. Check your connection and try again.";
