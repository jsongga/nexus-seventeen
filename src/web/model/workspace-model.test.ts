...
const task = (overrides: TaskOverrides = {}): BoardTask => {
  const estimateRecordedAt =
    overrides.estimateRecordedAt === undefined ? "2026-07-19T10:00:00.000Z" : overrides.estimateRecordedAt;
  const expectedCompletedAt =
    overrides.expectedCompletedAt === undefined ? "2026-07-19T10:15:00.000Z" : overrides.expectedCompletedAt;
...
describe("workspace view model", () => {
  ...
  it("includes worker connection state in assignment option labels", () => {
    expect(assignmentAgentOptionLabel(agent({ workerConnection: "waiting_for_wake" }))).toBe(
      "Engineer — Platform — Worker ready"
    );
    expect(assignmentAgentOptionLabel(agent({ workerConnection: "watching_run" }))).toBe(
      "Engineer — Platform — Worker connected"
    );
    expect(assignmentAgentOptionLabel(agent({ workerConnection: null }))).toBe(
      "Engineer — Platform — Worker not detected"
    );
  });
  ...
  it("derives implementation, review, and repeated-phase loops for an agent focus", () => {
    ...
    expect(agentPipelineFocus(agent({ role: "manager", currentTaskId: "task-one" }), [focusedTask])).toMatchObject({
      stage: "Reviewing",
    });
    expect(
      agentPipelineFocus(agent({ status: "running", currentTaskId: "review-task" }), [
        focusedTask,
        task({
          id: "review-task",
          kind: "manager_review",
    ...
    expect(
      agentPipelineFocus(focusedAgent, [
        task({
          status: "running",
          endedAt: null,
    ...
    expect(
      agentPipelineFocus(agent({ currentTaskId: null }), [task({ assignedAgentId: null, status: "backlog" })])
    ).toEqual({ task: null, phase: null, stage: null, loop: null });
    ...
    expect(agentPipelineFocus(focusedAgent, [parallelTask])).toMatchObject({
      phase: { id: "parallel-two" },
      loop: null,
    });
    ...
    expect(agentPipelineFocus(focusedAgent, [sequentialSameStage])).toMatchObject({
      phase: { id: "execution-b" },
      loop: null,
    });
    ...
    expect(agentPipelineFocus(focusedAgent, [reusedParallelGroup])).toMatchObject({
      phase: { id: "group-review" },
      loop: null,
    });
  ...
  it("uses absolute instants for phase, point-of-contact, resource, and update ordering", () => {
    ...
    expect(
      agentPipelineFocus(focusedAgent, [task({ status: "running", endedAt: null, phases: [earlierPhase, laterPhase] })])
        .phase?.id
    ...
    const orderedResources = resourcesForProject({ ...project, description: null, updatedAt: "2026-07-19T09:00:00Z" }, [
      task({
        id: "earlier-task",
        title: "Earlier task",
        workspaceRefs: ["https://example.com/shared"],
        updatedAt: earlierOffset,
        endedAt: earlierOffset,
      }),
      task({
        id: "later-task",
        title: "Later task",
  ...
  it("orders same-instant project updates independently of response order", () => {
    ...
    const messages = [
      ...
      },
      {
        ...
        body: "Omega",
        createdAt: offset,
        createdAtMs: Date.parse(offset),
      },
    ];
    const orderedIds = (input: typeof messages) =>
      recentUpdatesForProject(
        {
          revision: 1,
          generatedAt: offset,
  ...
  it("surfaces unassigned manager review and human release work as human attention", () => {
    ...
    expect(
      taskNeedsHumanAction(
        task({ kind: "human_check", assignedAgentId: null, assignedRole: null, status: "backlog", endedAt: null })
      )
    ).toBe(true);
    expect(
      taskNeedsHumanAction(
        task({ kind: "work", assignedAgentId: null, assignedRole: null, status: "backlog", endedAt: null })
      )
    ).toBe(false);
  });
  ...
  it("derives honest project links, setup references, and outcomes", () => {
    ...
    expect(resources.map((resource) => resource.kind)).toEqual(
      expect.arrayContaining(["brief", "link", "setup", "outcome"])
    );
    expect(resources.find((resource) => resource.kind === "link")?.href).toBe("https://example.com/runbook");
    expect(
      resourcesForProject(project, [task({ kind: "manager_review", requiredRole: "manager" })]).some(
        (resource) => resource.kind === "outcome"
      )
    ).toBe(false);
  });
