...
function runEvent(eventId: string, createdAt: string, taskId: string | null, data: Record<string, unknown>): RawEvent {
  return {
    eventId,
    projectId: project.projectId,
    taskId,
...
describe("run/event projection", () => {
  ...
  it("prefers an informative event over an uninformative event in both input orders", () => {
    const informative = runEvent("event-informative", "2026-08-09T10:02:00.000Z", "task-one", {
  ...
  it("prefers a fully populated event over a partially populated one at the same instant", () => {
    ...
    const claimed = runEvent("event-claimed", "2026-08-09T10:02:00.000Z", "task-one", { wakeReason: "assigned" });
  ...
  it("chooses the newest informative event by parsed milliseconds in both input orders", () => {
    const older = runEvent("event-older", "2026-08-09T12:02:00+02:00", "task-older", { wakeReason: "assigned" });
    const newer = runEvent("event-newer", "2026-08-09T10:02:00.500Z", "task-newer", { wakeReason: "resumed" });
