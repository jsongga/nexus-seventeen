...
export function deriveDecompositionAffordances(input: {
  ...
  return {
    approveAndMergeChildren: decomposedParent && !input.phasedFamily && input.workItemState === "final_approval",
    resumeCoordination:
      decomposedParent && input.workItemState === "parked" && !(input.phasedFamily && input.childFailed),
    resumeAfterBaseChange: input.workItemState === "parked" && input.parkCategory === "base_diverged",
    attestDeployment:
      input.parentWorkItemId !== null && attestablePhase && input.workItemState === "merged" && !input.deployAttested,
  };
}
...
export function contractApprovalIsReady(
  ...
  return phase === "contract" && state === "ready" && statuses.length > 0 && statuses.every((status) => status.ready);
}
...
export function contractDependencyStatuses(
  ...
  return siblings
    ...
    .sort(
      (left, right) =>
        (left.childOrdinal ?? Number.MAX_SAFE_INTEGER) - (right.childOrdinal ?? Number.MAX_SAFE_INTEGER) ||
...
export function deriveWorkItemDetailAffordances(input: {
  ...
  const terminal =
    input.workItemState === "merged" || input.workItemState === "dead_letter" || input.workItemState === "abandoned";
  if (terminal) {
    return {
      answerQuestion: false,
      confirmPlan: false,
...
export function proposedPlanForWorkItem(workflow: ProjectWorkflow, workItemId: string): DetailedWorkflowPlan | null {
  return (
    (workflow.plans as DetailedWorkflowPlan[])
...
export function pipelineFileReview(summary: PipelineSummary): Array<
  Readonly<{
    file: string;
...
export function pipelineAssumptionReview(summary: PipelineSummary): Array<
  Readonly<{
    assumption: string;
    addedMidRun: boolean;
  }>
...
export function pipelineFindingRounds(findings: readonly ReviewFinding[]): Array<
  Readonly<{
    round: number;
