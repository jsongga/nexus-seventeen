...
describe("missingRouteFallback", () => {
  ...
  it("canonicalizes a task id that has never appeared in an authoritative snapshot", () => {
    expect(missingRouteFallback({ kind: "tasks", taskId: "task-never-observed" }, snapshotIds, new Set())).toEqual({
      kind: "tasks",
    });
  });
  ...
  it("keeps a task route when the current snapshot contains the task", () => {
    expect(missingRouteFallback({ kind: "tasks", taskId: "task-present" }, snapshotIds, new Set())).toBeNull();
  });
});
