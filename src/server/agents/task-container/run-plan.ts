...
export function buildContainerRunPlan(input: {
  readonly options: Required<
    Pick<ContainerAgentLauncherOptions, "adapter" | "profile" | "model" | "image" | "networkName" | "proxyUrl">
  > &
  ...
  return {
    ...
    args: [
      ...
      containerName,
      "--label",
      `steward.task=${input.taskId}`,
      "--network",
      input.options.networkName,
      "--user",
      "node",
      "--cap-drop",
      "ALL",
      ...
      "/workspace",
      "-e",
      `HTTP_PROXY=${input.options.proxyUrl}`,
      "-e",
      `HTTPS_PROXY=${input.options.proxyUrl}`,
      "-e",
      `http_proxy=${input.options.proxyUrl}`,
      "-e",
      `https_proxy=${input.options.proxyUrl}`,
