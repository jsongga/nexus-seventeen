...
export function workItemFromRow(row: Row): WorkItem {
  ...
  return Object.freeze({
    ...
    ...(Object.hasOwn(row, "state_since") ? { stateSince: nullableString(row, "state_since") } : {}),
    ...(Object.hasOwn(row, "review_round") ? { reviewRound: nullableNumberValue(row, "review_round") } : {}),
    ...(Object.hasOwn(row, "heartbeat_at") ? { heartbeatAt: nullableString(row, "heartbeat_at") } : {}),
...
export function phaseFromRow(row: Row): TaskPhase {
  return Object.freeze({
    ...
    stage: stringValue(row, "stage") as PhaseStep,
