...
import type { ArtifactMediaType, CreateProjectArtifactRequest, ProjectArtifact } from "#shared/task-board-contract";
...
export class ArtifactStore {
  ...
  create(projectId: string, request: CreateProjectArtifactRequest, actor: string): ProjectArtifact {
    if (
      !MEDIA.has(request.mediaType) ||
      typeof request.caption !== "string" ||
      request.caption.trim() !== request.caption ||
      request.caption.length < 1 ||
      request.caption.length > 1_000
    ) {
    ...
    if (bytes.length < 1 || bytes.length > MAX_ARTIFACT_BYTES)
      throw new TaskBoardError(413, "ARTIFACT_TOO_LARGE", "Artifact is outside the supported size");
    const validContent =
    ...
    if (
      request.nodeId !== null &&
      !this.db.prepare("SELECT 1 FROM work_nodes WHERE node_id=? AND project_id=?").get(request.nodeId, projectId)
    ) {
      throw new TaskBoardError(400, "ARTIFACT_INVALID", "Artifact node is invalid");
    }
    if (
      request.taskId !== null &&
      !this.db.prepare("SELECT 1 FROM tasks WHERE task_id=? AND project_id=?").get(request.taskId, projectId)
    ) {
  ...
  list(projectId: string): readonly ProjectArtifact[] {
    return Object.freeze(
      (
        this.db
          .prepare("SELECT * FROM artifacts WHERE project_id=? ORDER BY created_at,artifact_id")
