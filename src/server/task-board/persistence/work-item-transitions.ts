...
export function registerParentTerminationCascade(store: TaskBoardStore, cascade: ParentTerminationCascade): void {
...
export function transitionWorkItemInTransaction(
  ...
  const requiresChild = fromState === "queued" && (request.to === "designing" || request.to === "implementing");
  ...
  const hasChildren =
    requiresDecomposedParent &&
    store.db.prepare("SELECT 1 FROM work_items WHERE parent_work_item_id=? LIMIT 1").get(request.workItemId) !==
      undefined;
  if ((requiresChild && row.parent_work_item_id === null) || (requiresDecomposedParent && !hasChildren)) {
    throw conflict(
      TASK_BOARD_ERROR_CODES.WORK_ITEM_ILLEGAL_TRANSITION,
      `work item cannot move ${fromState} -> ${request.to}`
    );
