...
export class NotificationsCollaborator {
  ...
  insertNotificationAtInTransaction(input: InsertNotificationInput, createdAt: string): BoardNotification | null {
    ...
    const next = this.runtime.store.db.prepare(`
    ...
    const inserted = this.runtime.store.db.prepare(`
    ...
    if (Number(inserted.changes) !== 1) {
      if (
        ...
        && this.runtime.store.db.prepare("SELECT 1 FROM notifications WHERE dedupe_key=?")
          .get(input.dedupeKey) !== undefined
      ) {
        return null;
      }
      throw new Error("TASK_BOARD_NOTIFICATION_INSERT_IGNORED");
    }
    const row = this.runtime.store.db.prepare("SELECT * FROM notifications WHERE notification_id=?")
  ...
  listNotifications(): NotificationList {
    const unread = this.runtime.store.db.prepare(`
      SELECT * FROM notifications
      WHERE read_at IS NULL
      ORDER BY sequence DESC
      LIMIT 100
    `).all().map((row) => notificationFromRow(row));
    const recentRead = this.runtime.store.db.prepare(`
      SELECT * FROM notifications
  ...
  markNotificationRead(notificationId: string, version: number): BoardNotification {
    return this.runtime.store.transaction(() => {
      const currentRow = this.runtime.store.db.prepare("SELECT * FROM notifications WHERE notification_id=?")
        .get(notificationId) as Row | undefined;
