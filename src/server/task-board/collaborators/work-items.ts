...
import { decompositionFamilyTouchesProjectSql, decompositionReadinessBlocker } from "./decomposition-readiness.js";
...
const PLANNING_ACCEPTANCE_CRITERIA_PREFIX =
  "Return a concise workflowPlan with explicit acceptance criteria, acyclic dependencies, and valid unique stage sequences. Available automated stages: ";
...
function onboardingUniqueConstraint(error: unknown): boolean {
  return (
    error instanceof Error && /UNIQUE constraint failed: work_item_onboarding_tasks\.project_id/u.test(error.message)
  );
}

export function workItemTitleProjection(workItem: Pick<WorkItem, "originalRequest" | "refinedObjective">): string {
...
export class WorkItemsCollaborator {
  ...
  setSuspendActiveRunInTransaction(
    suspend: (
      runId: string,
      reason: string,
      actor: { type: "human" | "agent" | "system"; id: string },
    ...
    const rows =
      tuple === null
        ? this.runtime.store.db
            .prepare(`${select} WHERE ${includeArchived ? "1=1" : "work_item.archived_at IS NULL"} ${orderAndLimit}`)
            .all()
        : this.runtime.store.db
            .prepare(
              `
  ...
  listChildren(parentWorkItemId: string): readonly ChildWorkItem[] {
    const completionByChild = new Map(
      this.parentCompletion(parentWorkItemId).children.map((child) => [child.workItemId, child.mergeSha] as const)
    );
  ...
  resumeDecomposedParent(workItemId: string): WorkItem {
    ...
    this.runtime.store.transaction(() => {
      ...
      const phasedFailure = this.runtime.store.db
        .prepare(
          `
        SELECT 1
  ...
  attestDeploy(workItemId: string, request: AttestDeployRequest): AttestDeployResult {
    ...
    const result = this.runtime.store.transaction(() => {
      ...
      const contractCandidates = this.runtime.store.db
        .prepare(
          `
      ...
      const readyContracts = contractCandidates.filter(
        (contract) =>
  ...
  dependenciesFor(workItemId: string): readonly WorkItemDependency[] {
    ...
    const dependencies = this.runtime.store.db
      .prepare(
        `
  ...
  requireWorkItem(workItemId: string): WorkItemDetail {
    ...
    const gapReport = this.runtime.store.db
      .prepare(
        `
    ...
    return Object.freeze({
      ...
      ...parkProjection,
      gapReportArtifactId:
        gapReport.gap_report_artifact_id === null ? null : stringValue(gapReport, "gap_report_artifact_id"),
    });
  }

  workItemAudit(workItemId: string): WorkItemAudit {
    this.runtime.requireWorkItem(workItemId);
    const gateActions = this.runtime.store.db
      .prepare(
        `
      SELECT *
      FROM gate_actions
    ...
    `
      ...
      .all(workItemId)
      .map((row) => this.gateActionFromRow(row));
    return Object.freeze({
  ...
  private workItemTransitions(workItemId: string): readonly WorkItemTransition[] {
    const transitions = this.runtime.store.db
      .prepare(
        `
    ...
    try {
      result = this.runtime.store.transaction(() => {
        ...
        if (created.workItem.state === "queued") {
          planning = this.startWorkItemPlanningInTransaction(
  ...
  private createWorkItemInternal(
    ...
    if (
      taskType === "onboarding" &&
      this.runtime.store.db.prepare("SELECT 1 FROM projects WHERE project_id=?").get(projectTarget.projectId) ===
        undefined
    ) {
    ...
    const apply = (): CreateWorkItemResult => {
      ...
      if (
        taskType === "onboarding" &&
        this.runtime.store.db
          .prepare("SELECT 1 FROM work_item_onboarding_tasks WHERE project_id=?")
  ...
  startWorkItemDesignInTransaction(workItemId: string): BoardTask | null {
    ...
    const nodes = this.runtime.store.db
      .prepare(
        `
      SELECT *
      FROM work_nodes
    ...
    `
      ...
      .all(String(confirmed.plan_revision_id))
      .map((node) => ({
        ...
        dependencyNodeIds: this.runtime.store.db
          .prepare(
            `
    ...
    const confirmedPlanRecord = {
      ...
      declaredScope:
        confirmed.declared_scope_json === null ? null : (JSON.parse(String(confirmed.declared_scope_json)) as unknown),
      nonGoals: confirmed.non_goals_json === null ? null : (JSON.parse(String(confirmed.non_goals_json)) as unknown),
      mechanicalPortions:
        confirmed.mechanical_portions_json === null
          ? null
          : (JSON.parse(String(confirmed.mechanical_portions_json)) as unknown),
      blockingQuestions:
        confirmed.blocking_questions_json === null
          ? null
          : (JSON.parse(String(confirmed.blocking_questions_json)) as unknown),
      criterionChecks:
        confirmed.criterion_checks_json === null
          ? null
  ...
  private startWorkItemPlanningInTransaction(
    ...
    const orphan = repairLegacyOrphan
      ? this.runtime.store.db
          .prepare(
            `
      SELECT task.task_id
    ...
    const now = exactNow(this.runtime.config.now);
    this.runtime.store.db
      .prepare("INSERT INTO work_item_planning_tasks VALUES(?,?,?)")
      .run(workItemId, task.taskId, now);
    if (onboarding) {
      const onboardingLink = this.runtime.store.db
        .prepare("SELECT 1 FROM work_item_onboarding_tasks WHERE work_item_id=?")
        .get(workItemId);
      if (onboardingLink === undefined) {
        ...
        this.runtime.store.db
          .prepare("UPDATE work_item_onboarding_tasks SET task_id=? WHERE work_item_id=?")
  ...
  updateWorkItem(workItemId: string, request: UpdateWorkItemRequest): WorkItem {
    ...
    return this.runtime.store.transaction(() => {
      ...
      if (request.projectTarget !== undefined && (current.state !== "queued" || current.parentWorkItemId !== null)) {
      ...
      const resolvedProjectId = request.projectTarget === undefined ? current.resolvedProjectId : targetProjectId;
      const now = exactNow(this.runtime.config.now);
  ...
  closeWorkItemWorkInTransaction(
    ...
    const linkedTasks = this.runtime.store.db
      .prepare(
        `
      SELECT task_id
      FROM (
    ...
    for (const linked of linkedTasks) {
      ...
      if (terminateActiveRuns) {
        ...
        for (const runId of runIds) {
          try {
            ...
            const durable = this.runtime.store.db
              .prepare(
                `
            ...
            `
              ...
              .get(runId);
            if (durable?.status !== "interrupted" || durable.ended_at === null || Number(durable.interrupted) !== 1) {
      ...
      if (!isHardTerminalTaskStatus(task.status)) {
        const taskUpdate = this.runtime.store.db
          .prepare(
            `
          UPDATE tasks
      ...
      const openQuestions = this.runtime.store.db
        .prepare("SELECT question_id FROM questions WHERE task_id=? AND status='open' ORDER BY asked_at,question_id")
        .all(task.taskId);
      const closedAnswer = `Closed because the work item was cancelled: ${persistedReason}`;
      const closedQuestions = this.runtime.store.db
        .prepare(
          `
        UPDATE questions
  ...
  private cancelChildrenForParentTerminationInTransaction(
    input: Readonly<{
    ...
    for (const child of children) {
      ...
      try {
        ...
        try {
          ...
          this.logChildCancellationFailure(input.parentWorkItemId, childWorkItemId, inspectionError, "fallback");
          continue;
        }
  ...
  private recordChildCancellationFailureInTransaction(
    ...
    try {
      ...
      this.logChildCancellationFailure(input.parentWorkItemId, childWorkItemId, recordingError, "recording");
    }
  }
  ...
  private logChildCancellationFailure(
    ...
    try {
      console.error(
        "[task-board] child cancellation cleanup failed",
        Object.freeze({
  ...
  private cancelWorkItemInTransaction(
    input: Readonly<{
      workItemId: string;
      reason: string;
      actor: { type: "human" | "agent" | "system"; id: string };
  ...
  private decompositionFamilyProjectIds(parentWorkItemId: string): ReadonlySet<string> {
    ...
    `
          ...
          .all(parentWorkItemId, parentWorkItemId) as Row[]
