...
export class MessagesCollaborator {
  ...
  askQuestion(taskId: string, agentId: string, request: CreateHumanQuestionRequest): HumanQuestion {
    ...
    const prior = this.runtime.store.db
      .prepare("SELECT * FROM questions WHERE agent_id = ? AND client_event_id = ?")
      .get(agentId, request.clientEventId);
    if (prior) {
      if (stringValue(prior, "request_hash") !== hash)
        throw conflict("CLIENT_EVENT_CONFLICT", "clientEventId was used for another question");
    ...
    this.runtime.store.transaction(() => {
      ...
      const settled = this.runtime.store.db
        .prepare(
          `
      ...
      if (task.status !== "blocked") {
        const blocked = this.runtime.store.db
          .prepare(
            `
          UPDATE tasks
        ...
        if (Number(blocked.changes) !== 1)
          throw conflict("TASK_VERSION_CONFLICT", "Task changed while its question was opening");
        this.runtime.insertEvent(
          task.projectId,
          taskId,
          { type: "agent", id: agentId },
          "task_blocked_for_human",
          {
            runId: request.runId,
            questionId,
      ...
      this.runtime.parkWorkItemForTaskInTransaction(taskId, { type: "agent", id: agentId }, now, {
        ...
        reason: persistedQuestion,
      });
      this.runtime.insertEvent(
        task.projectId,
        taskId,
        { type: "agent", id: agentId },
        "human_question_opened",
        {
          questionId,
          runId: request.runId,
        },
    ...
    return questionFromRow(
      this.runtime.store.db.prepare("SELECT * FROM questions WHERE question_id = ?").get(questionId)!
    );
  }
  ...
  answerQuestion(
    ...
    if (current.status === "answered") {
      ...
      const wake = this.runtime.store.db
        .prepare("SELECT * FROM wakeups WHERE reason = 'human_answer' AND source_key = ?")
  ...
  listMessages(taskId: string, after = 0): readonly TaskMessage[] {
    ...
    `
        ...
        .all(taskId, after, TASK_MESSAGE_PAGE_SIZE)
        .map(messageFromRow)
    );
  }
  ...
  private appendMessage(
    ...
    if (prior) {
      if (stringValue(prior, "request_hash") !== hash)
        throw conflict("CLIENT_EVENT_CONFLICT", "clientEventId was used for another message");
    ...
    return messageFromRow(
      this.runtime.store.db.prepare("SELECT * FROM task_messages WHERE message_id = ?").get(messageId)!
    );
  }
}
