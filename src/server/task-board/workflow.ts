...
export class TransparentWorkflow {
  ...
  async propose(raw: CreatePlanRevisionRequest, actor: string): Promise<ProjectWorkflowSnapshot> {
    ...
    const snapshots = await this.skills.load(raw.skillIds);
    ...
    const nodes = raw.nodes.map((node, index) => {
      ...
      if (stages.length === 0 || stages.at(-1) !== "verification") throw new TaskBoardError(400, "WORKFLOW_INVALID", "Every node must end in verification");
    ...
    this.transaction(() => {
      ...
      for (const node of nodes) {
        this.db.prepare("INSERT INTO work_nodes VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run(node.nodeId, planId, projectId, node.title, node.objective, JSON.stringify(node.acceptanceCriteria), JSON.stringify(node.stages), null, "pending", 1, createdAt, createdAt);
        for (const dependency of node.dependencyNodeIds) this.db.prepare("INSERT INTO work_node_dependencies VALUES(?,?)").run(node.nodeId, dependency);
      }
  ...
  confirm(planId: string, request: ConfirmPlanRevisionRequest, actor: string): readonly WorkNode[] {
    ...
    return this.transaction(() => {
      ...
      this.db.prepare("UPDATE work_items SET state='processing',resolved_project_id=?,current_stage='research',version=version+1,updated_at=? WHERE work_item_id=?").run(String(row.project_id), now, String(row.work_item_id));
  ...
  snapshot(projectId: string): ProjectWorkflowSnapshot {
    ...
    const nodes = plans.flatMap((plan) => this.nodes(plan.planRevisionId));
