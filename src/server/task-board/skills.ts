...
function frontmatter(source: string, expectedId: string): { name: string; description: string } {
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n/u.exec(source);
  if (!match) throw new TaskBoardError(500, "SKILL_INVALID", `Skill section ## ${expectedId} is missing YAML frontmatter`);
...
export class SkillRegistry {
  ...
  loadSync(skillIds: readonly string[]): readonly SkillSnapshot[] {
    ...
    for (const skillId of skillIds) if (!SKILL_ID.test(skillId)) throw new TaskBoardError(400, "SKILL_ID_INVALID", "Skill ID is invalid");
    const sections = this.#loadSections();
    return Object.freeze(skillIds.map((skillId) => {
      const content = sections.get(skillId);
      if (content === undefined || Buffer.byteLength(content) > MAX_SKILL_BYTES) throw new TaskBoardError(500, "SKILL_NOT_AVAILABLE", `Skill ${skillId} is unavailable`);
