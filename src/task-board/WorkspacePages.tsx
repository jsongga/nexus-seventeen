            ...
            {resources.length > 0 ? (
              <div className="grid gap-px bg-line md:grid-cols-2">
                {resources.map((resource) => {
                  ...
                  return resource.href ? <a key={resource.id} href={resource.href} target="_blank" rel="noopener noreferrer" className="flex min-h-28 gap-3 bg-white p-4 hover:bg-card">{content}</a> : <article key={resource.id} className="flex min-h-28 gap-3 bg-white p-4">{content}</article>;
                })}
              </div>
