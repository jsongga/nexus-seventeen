...
test("adds a discovered project and marks an existing Git project as already added", async ({ page }) => {
  ...
  await expect(source.getByRole("button", { name: "Your projects", exact: true })).toHaveAttribute(
    "aria-pressed",
    "true"
  );
...
test("browses from the reachable boundary and adds the current folder with an edited name", async ({ page }) => {
  ...
  await expect(breadcrumbs.getByRole("button", { name: "WebstormProjects", exact: true })).toHaveAttribute(
    "aria-current",
    "page"
  );
