...
test("reads documents and every event query inside one explicit snapshot", () => {
  const operations = [];
  const documentRows = [{ document_id: "snapshot-document-1" }, { document_id: "snapshot-document-2" }];
...
test("exports markdown and ordered event history with collision-safe slugs", async (t) => {
  ...
  assert.equal(await readFile(join(projectDirectory, "launch-plan-2.events.jsonl"), "utf8"), "");
});
...
test("rejects a post-drop database without document tables", async (t) => {
  ...
  assert.equal(result.stderr, "export-documents: database does not contain the documents table\n");
