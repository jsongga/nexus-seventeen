...
import { TASK_BOARD_API_VERSION, declaredScopesOverlap, type WorkItemDependency } from "#shared/task-board-contract";
import {
...
function phasedChildren(
  overrides: Partial<Record<"expand" | "migrateOne" | "migrateTwo" | "contract", Record<string, unknown>>> = {}
) {
  return [
    child("expand", {
...
test("phased declarations require exactly one expand, migrates, and exactly one contract", () => {
  assert.throws(
    () => parse("blast_radius", phasedChildren({ expand: { phase: "migrate", projectId: "consumer-three" } })),
    ContractValidationError
  );
  assert.throws(
    () => parse("blast_radius", phasedChildren({ migrateOne: { phase: "expand", projectId: "provider-project" } })),
    ContractValidationError
  );
  assert.throws(
    () =>
  ...
  assert.throws(() => parse("blast_radius", [phasedChildren()[0], phasedChildren()[3]]), ContractValidationError);
});

test("phase declarations are total and feature splits are unphased", () => {
  assert.throws(
    () => parse("blast_radius", phasedChildren({ migrateOne: { phase: undefined } })),
    ContractValidationError
  );
...
test("phased declarations pin direct expand and migrate dependency edges", () => {
  assert.throws(
    () => parse("blast_radius", phasedChildren({ migrateOne: { dependsOn: [] } })),
    ContractValidationError
  );
  assert.throws(
    () => parse("blast_radius", phasedChildren({ contract: { dependsOn: ["migrate-one"] } })),
    ContractValidationError
  );
});
...
test("phased projects keep expand and contract in the parent repo and migrates outside it", () => {
  ...
  assert.throws(
    () => parse("blast_radius", phasedChildren({ contract: { projectId: "consumer-three" } })),
    ContractValidationError
  );
  assert.throws(
    () => parse("blast_radius", phasedChildren({ migrateOne: { projectId: "provider-project" } })),
    ContractValidationError
  );
});
...
test("child dependency keys are unique, resolvable, acyclic, and duplicate-free", () => {
  assert.throws(
    () => parse("feature", [child("same"), child("same", { declaredScope: ["tests/same"] })]),
    ContractValidationError
  );
  assert.throws(() => parse("feature", [child("one", { dependsOn: ["missing"] })]), ContractValidationError);
  assert.throws(
    () => parse("feature", [child("one"), child("two", { dependsOn: ["one", "one"] })]),
    ContractValidationError
  );
  assert.throws(
    () =>
...
test("declared children do not invent a changeShape requirement", () => {
  assert.equal(
    parseWithoutChangeShape([child("one"), child("two", { projectId: "consumer-project", declaredScope: ["src/two"] })])
      ?.children?.length,
    2
  );
});
