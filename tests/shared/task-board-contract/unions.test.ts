...
test("agent statuses match the documented wire vocabulary", () => {
  assert.deepEqual([...AGENT_STATUSES], ["idle", "ready", "running", "interrupting", "waiting_for_human"]);
});
...
test("run statuses include active rather than running", () => {
  assert.deepEqual([...RUN_STATUSES], ["active", "waiting_for_human", "completed", "failed", "interrupted"]);
});
...
test("work item terminal states are absorbing", () => {
  assert.deepEqual([...WORK_ITEM_TERMINAL_STATES], ["merged", "abandoned", "dead_letter"]);

  for (const state of WORK_ITEM_STATES) {
    assert.equal(isTerminalWorkItemState(state), WORK_ITEM_TERMINAL_STATES.includes(state as never));
  }
...
test("illegal work item transitions have a stable error code", () => {
  assert.equal(TASK_BOARD_ERROR_CODES.WORK_ITEM_ILLEGAL_TRANSITION, "WORK_ITEM_ILLEGAL_TRANSITION");
});
