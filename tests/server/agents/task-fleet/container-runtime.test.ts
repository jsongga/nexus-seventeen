...
import { captureContainerRuntimeVersion } from "#server/agents/task-fleet/runtime";
...
test("captures the provider CLI label and immutable container image identity", async () => {
  ...
  const captured = await captureContainerRuntimeVersion("codex", "steward-agent:test", async (command, arguments_) => {
    ...
    return "1.2.3|sha256:abcdef0123456789abcdef\n";
  });

  assert.equal(captured, "1.2.3+abcdef012345");
...
test("treats Docker inspection failures and unsafe identity output as unavailable", async () => {
  ...
  assert.equal(
    await captureContainerRuntimeVersion("claude", "steward-agent:test", async () => (
      "1.2.3\u0000|sha256:abcdef0123456789abcdef\n"
    )),
    null,
  );
});
