...
CREATE TABLE gate_actions (
  ...
  gate TEXT NOT NULL CHECK (gate IN ('plan_confirm', 'plan_reject', 'final_approve', 'final_reject', 'cancel', 'question_answer')),
...
CREATE TABLE notifications (
  ...
  kind TEXT NOT NULL CHECK (kind IN ('park_aged', 'park_auto_abandoned', 'cap_parked', 'final_approval_withdrawn')),
...
CREATE TABLE park_records (
  ...
  work_item_id TEXT NOT NULL REFERENCES work_items(work_item_id) ON DELETE RESTRICT,
  category TEXT NOT NULL CHECK (category IN ('open_question', 'planning_run_failed', 'design_run_failed', 'hazardous_without_pipeline', 'plan_rejected_twice', 'bright_line', 'scope_violation', 'stage_cap_exceeded', 'task_cap_exceeded', 'base_diverged')),
  reason TEXT NOT NULL,
...
CREATE TABLE review_findings (
  ...
  file TEXT NULL,
  line INTEGER NULL,
  category TEXT NOT NULL CHECK (category IN ('correctness', 'security', 'plan_deviation', 'test_modification', 'docs', 'style', 'other')),
...
-- table: work_item_design_tasks
CREATE TABLE work_item_design_tasks (
  work_item_id TEXT PRIMARY KEY REFERENCES work_items(work_item_id) ON DELETE RESTRICT,
  task_id TEXT NOT NULL UNIQUE REFERENCES tasks(task_id) ON DELETE RESTRICT,
  created_at TEXT NOT NULL
) STRICT;

-- table: work_item_onboarding_tasks
CREATE TABLE work_item_onboarding_tasks (
...
CREATE TABLE work_item_transitions (
  ...
  from_state TEXT CHECK (from_state IS NULL OR from_state IN ('queued', 'planning', 'plan_approval', 'designing', 'implementing', 'verifying', 'reviewing', 'fixing', 'final_approval', 'merged', 'parked', 'abandoned', 'dead_letter')),
  to_state TEXT NOT NULL CHECK (to_state IN ('queued', 'planning', 'plan_approval', 'designing', 'implementing', 'verifying', 'reviewing', 'fixing', 'final_approval', 'merged', 'parked', 'abandoned', 'dead_letter')),
  actor_type TEXT NOT NULL CHECK (actor_type IN ('human', 'agent', 'system')),
  actor_id TEXT NOT NULL,
  created_at TEXT NOT NULL,
...
CREATE TABLE work_items (
  ...
  base_sha TEXT NULL,
  state TEXT NOT NULL CHECK (state IN ('queued', 'planning', 'plan_approval', 'designing', 'implementing', 'verifying', 'reviewing', 'fixing', 'final_approval', 'merged', 'parked', 'abandoned', 'dead_letter')),
  ...
  CHECK (
    (state IN ('merged', 'abandoned', 'dead_letter') AND ended_at IS NOT NULL) OR
    (state NOT IN ('merged', 'abandoned', 'dead_letter') AND ended_at IS NULL)
  )
) STRICT;
