...
import { TaskBoardError, createTaskBoardService, normalizeTaskBoardConfig } from "#server/task-board";
...
test("host routes are human-only and expose detected roots and safe directory listings", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      projectsBody.listing.entries.map((entry) => entry.name),
      ["alpha", "beta"]
    );
    ...
    assert.equal(
      (await get(address.url, `/v1/host/directories?path=${encodeURIComponent(`/${"a".repeat(512)}`)}`, HUMAN_TOKEN))
        .status,
      400
    );
  } finally {
