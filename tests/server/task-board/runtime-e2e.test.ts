...
const acmeAdapter: RuntimeAdapter = Object.freeze({
  ...
  args(_options: Parameters<RuntimeAdapter["args"]>[0], role: AgentRole, profile: RuntimeProfile): readonly string[] {
...
async function fixtureRepository(root: string): Promise<string> {
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "acme-runtime-fixture", private: true, type: "module" }, null, 2)}\n`
  );
    ...
    for (const agent of [
      {
        ...
        mission: "Plan the synthetic runtime change.",
        model: "acme-manager",
        token: MANAGER_TOKEN,
      },
      {
        ...
        mission: "Implement the synthetic runtime change.",
        model: "acme-engineer",
      ...
      },
      {
        ...
        mission: "Review the synthetic runtime change.",
        model: "acme-verifier",
...
test("the test-local Acme adapter maps its distinct line protocol", () => {
  ...
  assert.deepEqual(acmeAdapter.events('EVT {"kind":"say","text":"hello"}'), [{ type: "message_delta", text: "hello" }]);
  assert.deepEqual(acmeAdapter.events('EVT {"kind":"call","cmd":"file_change","detail":"src/acme/result.txt"}'), [
