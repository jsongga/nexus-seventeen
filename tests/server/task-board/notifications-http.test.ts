...
test("human notification endpoints bound lists and mark rows read with CAS", async () => {
  ...
  try {
    ...
    assert.equal(listed.unread.every((notification) => notification.readAt === null), true);
    ...
    assert.equal(listed.recentRead.every((notification) => notification.readAt !== null), true);

    const markedResponse = await request(
      address.url,
      "/v1/notifications/notification-154/read",
      "POST",
      HUMAN_TOKEN,
      { version: 1 },
    );
    assert.equal(markedResponse.status, 200);
    const marked = (await markedResponse.json() as {
    ...
    const unknown = await request(
      address.url,
      "/v1/notifications/notification-unknown/read",
      "POST",
      HUMAN_TOKEN,
      { version: 1 },
    );
    ...
    assert.equal((await request(
      address.url,
      "/v1/notifications/notification-153/read",
      "POST",
      HUMAN_TOKEN,
