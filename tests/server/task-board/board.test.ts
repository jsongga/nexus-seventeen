...
test("work items preserve explicit intake and enforce idempotent CAS updates", async () => {
  ...
  try {
    ...
    assert.equal(Number(direct.prepare("PRAGMA user_version").get()?.user_version), 23);
  } finally {
    direct.close();
  }
...
test("schema version 9 migration adds dormant automation configuration without changing existing board state", async () => {
  ...
  const verified = new DatabaseSync(path);
  try {
    assert.equal(Number(verified.prepare("PRAGMA user_version").get()?.user_version), 23);
