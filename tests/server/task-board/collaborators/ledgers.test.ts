...
import { AGENT_ONE_TOKEN, HUMAN_TOKEN, boardFixture, databasePath, taskRequest, workItemRequest } from "../helpers.js";
...
test("ledger collaborators aggregate findings, filter by project, and order park history", async () => {
  ...
  try {
    ...
    const firstItem = fixture.board.createWorkItem(
      workItemRequest({
        originalRequest: "Original first work-item request.",
        projectTarget: { mode: "explicit", projectId: fixture.project.projectId },
      }),
      "ledger-work-item-one"
    ).workItem;
    ...
    try {
      db.prepare("UPDATE work_items SET refined_objective=? WHERE work_item_id=?").run(
        "Refined first work-item objective.",
    ...
    assert.deepEqual(
      [...findings.perProject].sort(
        (left, right) => left.projectId.localeCompare(right.projectId) || left.category.localeCompare(right.category)
      ),
      [
    ...
    assert.deepEqual(
      findings.recent.slice(0, 3).map((finding) => finding.findingId),
      ["ledger-finding-two-a", "ledger-finding-two-b", "ledger-finding-two-c"]
    );
    assert.equal(
      findings.recent.every((finding) => finding.workItemId.length > 0),
      true
    );
    ...
    assert.equal(
      filtered.recent.every((finding) => finding.workItemId === firstItem.workItemId),
      true
    );

    const parks = fixture.board.parksLedger();
    assert.deepEqual(
      parks.open.map((record) => record.parkRecordId),
      ["park-open-older", "park-open-newer"]
    );
    assert.deepEqual(
      parks.resolved.map((record) => record.parkRecordId),
      ["park-resolved-newer", "park-resolved-older"]
    );
