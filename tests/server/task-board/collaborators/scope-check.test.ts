...
test("declared-scope matching is exact at prefix boundaries", async (t) => {
  const cases = [
    ...
    { name: "file is rooted below prefix", changed: "src/server/index.ts\0", scope: ["src"], expected: { ok: true } },
    {
      ...
      changed: "src/server/index.ts\0",
      scope: ["src/"],
      expected: { ok: true },
    },
    {
      name: "file is outside every prefix",
      changed: "docs/design.md\0",
      scope: ["src", "tests"],
      expected: { ok: false, files: ["docs/design.md"] },
    },
    {
      name: "adjacent prefix does not match",
      changed: "srcx/server/index.ts\0",
      scope: ["src"],
      expected: { ok: false, files: ["srcx/server/index.ts"] },
    },
    {
...
test("scope-violation results are bounded to the settlement limit", () => {
  const result = scopeViolationResult(
    Array.from({ length: 300 }, (_, index) => `outside/${index.toString().padStart(3, "0")}-${"x".repeat(20)}.ts`)
  );
