...
async function linkedWorkFixture() {
  ...
  store.db.prepare("INSERT INTO work_item_planning_tasks VALUES(?,?,?)").run(workItemId, taskIds[0], CREATED_AT);
  store.db.prepare("INSERT INTO work_item_design_tasks VALUES(?,?,?)").run(workItemId, taskIds[1], CREATED_AT);
  store.db
    .prepare(
      `
...
test("closeWorkItemWorkInTransaction cancels every linked task and is idempotent", async () => {
  ...
  try {
    fixture.store.transaction(() =>
      fixture.workItems.closeWorkItemWorkInTransaction(
        fixture.workItemId,
    ...
    `
        ...
        .map((row) => ({ ...row })),
      fixture.taskIds
        .filter((taskId) => taskId !== fixture.taskIds[1])
        .map(() => ({
          status: "answered",
    ...
    for (const taskId of fixture.taskIds) {
      assert.ok(
        fixture.store.db
          .prepare("SELECT 1 FROM task_events WHERE event_id='retired-wakeup:' || ?")
    ...
    const beforeReplay = {
      ...
      taskVersions: fixture.store.db
        .prepare("SELECT version FROM tasks ORDER BY order_key")
        .all()
        .map((row) => row.version),
      questionVersions: fixture.store.db
        .prepare("SELECT version FROM questions ORDER BY question_id")
        .all()
        .map((row) => row.version),
    };
    fixture.store.transaction(() =>
