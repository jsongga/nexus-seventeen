...
function pipelinePlan(tier: "standard" | "hazardous" = "standard", suffix = "default"): WorkflowPlanDraft {
  return {
...
async function startPipeline(fixture: Awaited<ReturnType<typeof capFixture>>, suffix: string) {
  ...
  try {
    const attempt = inspected
      .prepare("SELECT attempt_id FROM stage_attempts WHERE task_id=?")
...
test("stage clock uses retry boundaries and task clock sums post-resume run intervals", () => {
  ...
  try {
    ...
    db.prepare("INSERT INTO park_records VALUES('item-1',?,'resumed')").run("2026-08-21T11:00:00.000Z");
  ...
  try {
    ...
    assert.equal(
      fixture.board
        ...
        .recentRuns.find((run) => run.runId === active.implementation.run.runId)?.status,
      "interrupted"
    );
...
test("wall-clock caps ignore a coordinating parent's stray active planning run", async () => {
  ...
  try {
    ...
    assert.equal(fixture.board.requireWorkItem(workItem.workItemId).state, "coordinating");
    assert.equal(
      fixture.board.snapshot(fixture.project.projectId).recentRuns.find((run) => run.runId === planning.run.runId)
        ?.status,
      "active"
    );
  } finally {
...
test("a cap-parked stage stays blocked through reconciliation and human retry reactivates it", async () => {
  ...
  try {
    ...
    try {
      attemptCount = Number(
        before.prepare("SELECT COUNT(*) AS count FROM stage_attempts WHERE node_id=?").get(active.node.nodeId)?.count
      );
      taskCount = Number(
        before
          .prepare(
    ...
    try {
      assert.equal(
        Number(
          reconciled.prepare("SELECT COUNT(*) AS count FROM stage_attempts WHERE node_id=?").get(active.node.nodeId)
            ?.count
        ),
        attemptCount
      );
...
test("cap notifications use the sweep's injected timestamp", async () => {
  ...
  try {
    ...
    try {
      assert.equal(
        inspected
          .prepare("SELECT created_at FROM notifications WHERE dedupe_key=?")
          .get(`cap_parked:${active.workItem.workItemId}:${active.attemptId}`)?.created_at,
        sweepNow
      );
    } finally {
      inspected.close();
...
test("task cap sums genuine run time across stage retries within one resumed epoch and parks once", async () => {
  ...
  try {
    ...
    const inspected = new DatabaseSync(fixture.path, { readOnly: true });
    try {
      assert.equal(taskActiveSeconds(inspected, active.workItem.workItemId, at(1_030).toISOString()), 70);
    } finally {
      inspected.close();
    }
  ...
  try {
    ...
    try {
      ...
      `
          ...
          .get(active.workItem.workItemId)?.count,
        0
      );
    } finally {
      db.close();
...
test("disabled caps leave active work untouched", async () => {
  ...
  try {
    ...
    assert.equal(
      fixture.board
        .snapshot(fixture.project.projectId)
        .recentRuns.find((run) => run.runId === active.planning.run.runId)?.status,
      "active"
    );
...
test("a scope-held item aged four hours with no runs accrues no task-cap time", async () => {
  ...
  try {
    ...
    try {
      db.prepare("DELETE FROM project_events WHERE node_id=? AND event_type='node_blocked'").run(held.node.nodeId);
      db.prepare("UPDATE work_nodes SET state='ready',version=version+1 WHERE node_id=?").run(held.node.nodeId);
      db.prepare(
        `
    ...
    const blocked = fixture.board
      .projectWorkflow(fixture.project.projectId)
      .nodes.find((candidate) => candidate.nodeId === held.node.nodeId);
    assert.equal(blocked?.state, "blocked");
    const latestBlock = fixture.board
      .listProjectEvents(fixture.project.projectId)
      .findLast((event) => event.nodeId === held.node.nodeId && event.eventType === "node_blocked");
