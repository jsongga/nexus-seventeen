...
function boundedText(value, maximum, path) {
  assert(
    ...
    `${path} must be 1-${maximum} trimmed characters`
  );
}
...
export function projectDescription(project) {
  return [
    ...
    ...project.resources.map((resource) => `${resource.label}: ${resource.value}`),
  ].join("\n");
}
...
export function expandedAgentProfiles(catalog) {
  return catalog.projects.flatMap((project) =>
    catalog.projectAgentProfiles.map((profile) => ({
...
export function validateCatalog(catalog) {
  ...
  assert(
    Array.isArray(catalog.projectAgentProfiles) && catalog.projectAgentProfiles.length > 0,
    "projectAgentProfiles must be a nonempty array"
  );
  assert(
    Array.isArray(catalog.agentTypes) && catalog.agentTypes.length <= 32,
    "agentTypes must contain at most 32 entries"
  );
  assert(
    Array.isArray(catalog.stages) && catalog.stages.length === stageOrder.length,
    "stages must contain every canonical stage"
  );
  ...
  for (const [index, project] of catalog.projects.entries()) {
    ...
    for (const [resourceIndex, resource] of project.resources.entries()) {
      ...
      assert(
        !/[\r\n]/u.test(resource.label + resource.value),
        `projects[${index}].resources[${resourceIndex}] cannot contain line breaks`
      );
    }
  ...
  for (const [index, profile] of catalog.projectAgentProfiles.entries()) {
    ...
    assert(
      /^[a-z0-9][a-z0-9-]*$/u.test(profile.suffix),
      `projectAgentProfiles[${index}].suffix must be lowercase kebab-case`
    );
  ...
  for (const [index, agentType] of catalog.agentTypes.entries()) {
    ...
    assert(
      typeof agentType.supplementalInstructions === "string" && agentType.supplementalInstructions.length <= 8_000,
      `agentTypes[${index}].supplementalInstructions is invalid`
    );
    assert(
      !agentType.enabled || agentType.supplementalInstructions.trim().length > 0,
      `enabled agentTypes[${index}] requires instructions`
    );
    assert(
      Array.isArray(agentType.skillIds) && agentType.skillIds.length <= 32,
      `agentTypes[${index}].skillIds is invalid`
    );
    assert(
      agentType.skillIds.every((skillId) => skillIdentifierPattern.test(skillId)),
      `agentTypes[${index}] has an invalid skill ID`
    );
    assert(
      new Set(agentType.skillIds).size === agentType.skillIds.length,
      `agentTypes[${index}] has duplicate skill IDs`
    );
  ...
  for (const [index, entry] of catalog.stages.entries()) {
    ...
    assert(
      entry.executor !== null && typeof entry.executor === "object" && !Array.isArray(entry.executor),
      `stages[${index}].executor must be an object`
    );
  ...
  const aggregateBytes = Buffer.byteLength(
    JSON.stringify({ agentTypes: catalog.agentTypes, stages: catalog.stages }),
    "utf8"
  );
...
export function mergeAutomationConfiguration(remote, desiredAgentTypes, desiredStages) {
  ...
  for (const existing of remote.agentTypes) {
    ...
    if (desired && desired.role !== existing.role) {
      throw new Error(
        `Automation drift: ${existing.agentTypeId} role is immutable (${existing.role}, wanted ${desired.role})`
      );
    }
  }
